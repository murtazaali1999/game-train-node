...
module.exports = {
  createChallenge: async (name,numberOfLevels) => {
    ...
    const tx = await program.rpc.createChallenge(name, numberLevels, {
      accounts: {
  ...
  initChallenge: async (name,numberOfLevels) =>{
    ...
    const tx = await program.rpc.initializeChallenge(
      name,Levels,{
      accounts: {
        challenge: challengePubKey,
        challangeSigner: wallet.publicKey,
  ...
  initPlayer: async (player,playerSigner) =>{
    ...
    const [challengePubKey, challengeBump] =
      ...
      const [playerPubKey, playereBump] =
      await web3.PublicKey.findProgramAddress(
        [
    ...
    const tx = await program.rpc.initializePlayer(
        {
      accounts: {
  ...
  withdrawPlatformMoney: async () =>{
    ...
    const tx = await program.rpc.platformWithdraw(
        {
      accounts: {
        challangeSigner: wallet.publicKey,
        challenge: challengePubKey
  ...
  awardPointsToWinner: async (player,signer) =>{
    ...
    const tx = await program.rpc.awardPoints(
        {
      accounts: {
        player:playerPubKey,
        signer,
  ...
  enterChallenge: async (player,user) => {
    ...
    const tx = await program.rpc.enterChallenge(
        {
      accounts: {
        player,
        user,   //signer
  ...
  withdrawPrizeMoney: async (challangeSigner) => {
    ...
    const tx = await program.rpc.winnerWithdraw(
        {
      accounts: {
        challenge: challengePubKey,
        challangeSigner,
